...
public readonly struct MaskValue
{
    ...
    public MaskValue(byte packed) => this.Expanded = Unpack(packed) * 0xFFu;
    ...
    /// </summary>
    public byte Packed => (byte)Pack(this.Expanded);

    public override string ToString() => $"0x{this.Packed:x2}";
