...
public class TextConsole
{
    ...
    {
        Point cursorPos = this.CursorPosition;
    ...
    {
        int code = 0;
        if (args?.Length > 0 && !int.TryParse(args[0].Trim(), out code)) {
            code = 0;
        }
    ...
    private void AnsiGraphics(string[]? args)
    {
        ...
        if (args?.Length > 1 && int.TryParse(args[1].Trim(), out code)) {
            this.RunGraphicsCommand(code);
        }
    }
