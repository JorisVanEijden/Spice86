...
public abstract class VideoMode {
    ...
    private void InitializeFont(Memory memory) {
        (ushort offset, int length) = FontHeight switch {
            8 => (Memory.Font8x8Offset, 8 * 256),
            14 => (Memory.Font8x14Offset, 16 * 256),
            16 => (Memory.Font8x16Offset, 16 * 256),
            _ => throw new InvalidOperationException("Unsupported font height.")
        };

        Span<byte> span = memory.GetSpan(Memory.FontSegment, offset, length);
        span.CopyTo(Font);
    }
}
