...
public sealed class AdlibGold : OPL3FM {
    ...
    public AdlibGold(Machine machine, Configuration configuration, ushort _sample_rate) : base(machine, configuration) {
        sample_rate = _sample_rate;
        stereo_processor = new(sample_rate);
    ...
    private class StereoProcessor {
        ...
        private ILogger _logger = Spice86.Logging.Serilogger.Logger.ForContext<StereoProcessor>();

        public StereoProcessor(ushort _sample_rate) {
            sample_rate = _sample_rate;
            if (sample_rate <= 0) {
